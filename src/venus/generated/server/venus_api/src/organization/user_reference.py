...
class _Factory:
    def user_id(self, value: UserId) -> UserReference:
        return UserReference(
            __root__=_UserReference.UserId(type="userId", user_id=value)
        )

    def email_address(self, value: str) -> UserReference:
        return UserReference(
            __root__=_UserReference.EmailAddress(
                type="emailAddress", email_address=value
            )
        )
...
class UserReference(pydantic.BaseModel):
    ...
    def visit(
        self,
        user_id: typing.Callable[[UserId], T_Result],
        email_address: typing.Callable[[str], T_Result],
    ) -> T_Result:
